 ...
 * XML format for {@link AbstractModel} objects, which are used as resources in Sutton framework. It is also responsible
 * for the deserialization process in order to recreate the {@link Link} from XML sent in the request body.
 */
public class XmlServerLinkConverter extends XmlAdapter<ServerLink, Link> {
	@Override
	public Link unmarshal(ServerLink v) throws Exception {
		if (v != null) {
	...
	@Override
	public ServerLink marshal(Link v) throws Exception {
		if (v != null) {
